...
`generate_formula_renders` guarded parse-*time* failures and then accepted any
non-`None` render. `"<parse error: invalid syntax>"` is not `None`, so twelve
formulas — including `w0-derivation`, `alpha-t-derivation` and
`dark-matter-abundance` — shipped that string to the site **as their EML
rendering**, counted as successes. Three more published the parser-internal
name `b3_leaf`, and `b3_leaf` was visible inside 92 further renders.

- `generators/eml_render_validity.py` decides whether a render is showable,
  and returns a reason rather than a boolean. The EML option is withheld when
  there is nothing valid to show, which needed no front-end change:
  `pm-formula-multi-render` already attaches nothing when a formula has no
  entry.
- Policy chosen by running **both** candidate policies against the live
  formula set, not by preference. The permissive one would have published
  `8` as the EML form of `G₂ = Aut(𝕆)` and `1` for `Hol(g) ⊆ G₂ ⟺ ∃η`. The
  losing branch is preserved as an evidence table and a pinned test.
- An earlier classifier of mine counted `<text` elements and called anything
  under three "trivial". That threshold was invented and produced ten false
  positives; it is replaced by a structural question — does the tree contain
  an operator?
- 10 of the 12 diagnosed formulas recovered: their `eml_tree_str` is a
  comment block while the `eml_description` beside it holds the complete
  expression. Renders now 399/422, with zero published error text and zero
  leaked internal names.
- Build tripwire: if *every* formula fails, the build errors — that is a
  broken renderer, not N broken formulas.

### Fixed — two silent wrong-directory bugs

- `build()` ignored `METAPHYSICA_OUT` and then overwrote it for child steps,
  so `METAPHYSICA_OUT=/site python -m metaphysica.build` wrote the site into
  the cwd while `/site` kept its old contents — a build that reports success
  and leaves you inspecting stale artifacts.
- `simulations/AutoGenerated` was a Windows junction pointing at a dead temp
  directory. The binder replaced a POSIX symlink with a wrong target, but a
  junction reports `is_symlink() == False`, so it fell into the "real
  directory, don't clobber" branch and only warned — every build.

### Added — bibliography connected to the claims

228 of 230 references were orphans: defined, cited by nothing.
`Formula.references` existed in the schema and no simulation ever set it.
Formulas and parameters now carry the reference ids their own module
registered, joined on `source_simulation`. This is **module-level**
attribution and is flagged as such (`references_are_module_level`) — it says
"this formula comes from a module that declared these papers", not "this
paper proves this line". `generators/generate_reference_integrity.py` audits
identifiability, duplication and citation, and runs in the build.

### Added — the coupling graph, and a correction

The 21 allowed couplings on the 21 coordinate pairs form a 2-regular graph:
...
minimum is five live couplings, never zero). It was an unfalsifiable guard,
the exact defect this project audits out of its own gates, and it shipped.
...
- `test_artifact_freshness.py` — shipped artifacts are **recomputed** and
  compared. Twice this cycle a stale artifact was mistaken for a live one.
- `test_falsifiers_are_fireable.py` — a declared falsifier must describe
  something that can happen. Written because one here did not.
- `test_topological_terms::test_face_triangle_identification` — the decisive
  test for the face-triangle proposal, ready and skipping with the exact
  contract that activates it.

### Assessed — not silently repaired

`docs/SCALE_DISAGREEMENTS.md` examines the five `DISAGREE_SCALE` rows; only
one is a unit slip. `portals.sterile_mixing_sin2_2theta` is not a scale error
at all: its description claims sin²(2θ) = 1/b₃ = 0.0417, which exceeds the
sterile-mixing bound, while the registered 4.28e-05 passes — **a falsified
claim published as the description of a passing parameter** (register 1.6a).
Four of the five could be made to "agree" by editing the `eml_description`;
that would be making a check pass by editing the thing being checked, so the
correction is left on whichever side is actually wrong.
