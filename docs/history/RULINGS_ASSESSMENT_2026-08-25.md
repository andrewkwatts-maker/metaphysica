# SUPERSEDED — historical record, not live guidance

This document was written on 2026-08-25 on the `continue/gates-and-rulings`
branch and never merged. It is preserved here under the standing rule that
nothing is silently retired. **It must not be used to decide any ruling.**
Three reasons, each independently sufficient:

1. **Its method conflicts with the anti-tuning guard.** It scores each option on
   an "Accuracy vs data" axis (1–5) and issues recommendations. The framework's
   own `switch_search` returns the constant verdict `NO_SELECTION_MADE`
   precisely because "a switchboard that reports 'which option fits best' is a
   parameter fitter", and `tests/test_variants.py` bans the words
   best/score/rank/sigma/preferred/recommended from fork options. This document
   predates both disciplines. Ranking theory branches by agreement with
   experiment is the thing the project decided not to do.

2. **Its central premise has since been refuted.** Ruling (a) scores
   `sin(theta_13) = 1/sqrt(b_3)` 5/5 on "Geometric derivation" because it
   "follows directly from b_3 = 24". b_3 = 24 is now an INPUT with its origin
   open, and is unreachable by a Joyce (Z/2)^3 resolution, which reaches only
   b_3 ≡ 7 (mod 12).

3. **Its numbers are stale.** Ruling (e) compares three values of Sigma m_nu;
   there are now four, three of them published DERIVED, spread 2.4×. The
   validation board it reasons against has also moved (179 records → 204,
   13 FAIL → 20).

What remains useful is the *inventory*: the seven questions it raises — theta13,
the S8 branch, the n_s branch, the H0 anchor, the neutrino mass sum, the
G12/G30/G32 tolerances, and G36 CKM unitarity — are all still open in the
register. **The questions survive; the scoring does not.**

The original text follows verbatim, quoted.
