...
def test_sterility_report_has_exactly_one_writer():
    """Guards the 2026-08-24 single-writer fix.

    sync_docs.py and verify_sterility_report.py both wrote
    AutoGenerated/sterility_report.json in DIFFERENT schemas -- flat vs
    metadata-block -- and both run inside run_all_simulations (lines 3414 and
    3434). Last one won, so the artifact flipped layout depending on the code
    path and churned 160+ lines on every regeneration. The duplicate writer in
    sync_docs was removed; this fails if a second one reappears.

    Notably the artifact has NO consumers -- nothing in src/, tests/, scripts/
    or the website reads it -- so the churn was pure git noise. That is an
    argument for one writer, not for two writers kept in sync.
    """
    import io
    from pathlib import Path
    ...
    for path in src.rglob("*.py"):
        ...
        for lineno, line in enumerate(text.splitlines(), 1):
            code = line.split("#", 1)[0]
            if "sterility_report.json" not in code:
                continue
            # a write site assigns the path or passes it to a write call
            if any(tok in code for tok in ("write_report(", "output_path", "/ \"sterility_report.json\"")):
                writers.append(f"{path.name}:{lineno}")
    assert len(writers) == 1, (
        f"sterility_report.json must have exactly one writer, found: {writers}"
    )
