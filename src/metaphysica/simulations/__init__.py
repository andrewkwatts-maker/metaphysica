...
def _bind_output_dir():
    ...
    #
    # `legacy.exists()` FOLLOWS the link, so it is False for a DANGLING one --
    # a link whose target has been deleted. That is not hypothetical: pointing
    # METAPHYSICA_OUT at a temp dir and then removing the temp dir leaves
    # exactly that, and the repair below was skipped entirely, so the broken
    # link survived every later run. `is_symlink()` is True for a dangling
    # link, which is the distinction needed. Same family as the junction bug
    # described below, one case further out.
    # A DANGLING link -- one whose target has been deleted -- has to be caught
    # before the exists() branch, because exists() FOLLOWS the link and is
    # therefore False for exactly this case, so the repair below never ran and
    # the broken link survived every later run. Not hypothetical: pointing
    # METAPHYSICA_OUT at a temp dir and then deleting the temp dir produces it.
    #
    # is_symlink() is not reliable here either -- a Windows junction, and a
    # POSIX symlink made by Git Bash, both report False. os.readlink succeeds
    # on links and junctions and raises OSError on a real directory, which is
    # the distinction that actually holds on both platforms (the same trick
    # this function already uses below).
    if _os.path.lexists(legacy):
        try:
            _dangling_target = _os.readlink(legacy)
        except OSError:
            _dangling_target = None
        if _dangling_target is not None and not _Path(_dangling_target).exists():
            try:
                legacy.unlink()
            except OSError:
                try:
                    legacy.rmdir()      # junctions unlink via rmdir
                except OSError:
                    print(f"[metaphysica] WARNING: {legacy} is a dangling link "
                          f"to {_dangling_target} and could not be removed",
                          file=_sys.stderr)
                    return
