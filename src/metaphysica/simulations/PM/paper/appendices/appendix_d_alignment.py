...
#: The v24.2 "0.48 sigma global alignment" literal. NOT a computed statistic:
#: see the module docstring and the note in run(). The computed global fit is
#: chi^2 over the scoring rows in AutoGenerated/statistical_rigor_report.json
#: and is very far from this. Kept, named, and labelled rather than silently
#: defaulted; replacing it with the computed figure is an author ruling.
SIGMA_GLOBAL_LEGACY_LITERAL = 0.48
...
class AppendixDAlignment(SimulationBase):
    ...
    def run(self, registry: 'PMRegistry') -> Dict[str, Any]:
        ...
        # No defaults: both paths exist, so a default here could only ever
        # be dead weight that hides a missing dependency if one is ever
        # dropped.
        H0_geo = registry.get("cosmology.H0_local")
        w0_geo = registry.get("cosmology.w0_derived")
        ...
        # This module PRODUCES validation.sigma_global (see the return
        # below), so reading it from the registry was reading its own output
        # before writing it: the default fired every time. Naming the literal
        # removes the false provenance without changing the number, which is
        # the author ruling recorded in the note above.
        sigma_global = SIGMA_GLOBAL_LEGACY_LITERAL
    ...
    def get_output_param_definitions(self) -> List[Parameter]:
        ...
        return [
            Parameter(
                ...
                # EML WITHHELD: this is the v24.2 literal 0.48 squared. sigma_global is
                # in no registry, so the module default always fires and the value is
                # 0.2304 by construction, not by measurement. The computed global fit
                # lives in statistical_rigor_report.json (chi_squared_from_rows). No EML
                # derivation is claimed for a superseded legacy scalar.
