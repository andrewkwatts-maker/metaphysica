...
class FalsificationOracle:
    ...
    def _live_registry_value(self, path: str, fallback: float) -> float:
        """Read a prediction from the live build registry when available.

        2026-08 audit fix: the oracle compared hardcoded predictions to
        hardcoded limits, so its kill-switch report could drift from the
        actual theory (its tau_p once contradicted cert G23). Registry
        values now override the literals; the literals remain offline
        fallbacks only.
        """
        try:
            ...
            base = _Path(_os.environ.get("METAPHYSICA_OUT", "."))
            params = _json.loads(
                (base / "AutoGenerated" / "parameters.json").read_text(encoding="utf-8")
            ).get("parameters", {})
            v = params.get(path, {}).get("value")
    ...
    def _load_predictions(self) -> Dict[str, Dict[str, float]]:
        ...
        Numeric predictions overlay from the live registry (see
        _live_registry_value); literals are offline fallbacks.
        """
        return {
            ...
            "proton_decay": {
                "tau_p_years": self._live_registry_value(
                    "proton_decay.lifetime_years", 4.757e34),  # LIVE registry value
                "mode": "p → π⁰ e⁺",
