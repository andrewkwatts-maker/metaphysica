...
def gate_G32_wz_gut_ratio() -> GateResult:
    ...
    # Routed through _params_path() like every other gate here. It used to
    # call autogen_dir() directly, which (a) skips the bundled-wheel fallback
    # that exists precisely so a gate is always scorable, and (b) MKDIRS the
    # directory as a side effect of reading -- so merely evaluating this gate
    # created an empty repo-local AutoGenerated/ that then shadowed the real
    # published tree for every artifact test in the process.
    with open(_params_path(), encoding="utf-8") as fh:
        params = json.load(fh)["parameters"]
