...
(function(global) {
    ...
    // Configuration
    //
    // Simulation source files live in the `metaphysica` PyPI lib repo
    // (https://github.com/andrewkwatts-maker/metaphysica) under
    // `src/metaphysica/<index-relative-path>`. The PrincipiaMetaphysica
    // repo is a pure generated-output tree and no longer contains any
    // `.py` files, so the legacy GITHUB_REPO pointing at it 404'd on
    // every fetch ("Failed to load file content").
    const CONFIG = {
        GITHUB_REPO: 'andrewkwatts-maker/metaphysica',
        GITHUB_BRANCH: 'main',
        // simulations-index emits paths relative to the lib's source root
        // (e.g. "simulations/PM/algebra/_triple_helpers.py"); prepend the
        // wheel-layout prefix so the raw URL hits the right blob.
        SOURCE_PREFIX: 'src/metaphysica/',
        INDEX_PATHS: [
    ...
    /**
     * Prepend the lib's source prefix to an index-relative path.
     * Idempotent: paths that already start with SOURCE_PREFIX are
     * returned unchanged.
     */
    function libPath(path) {
        if (!path) return path;
        const p = String(path).replace(/^\/+/, ''); // strip leading slashes
        if (p.startsWith(CONFIG.SOURCE_PREFIX)) return p;
        return CONFIG.SOURCE_PREFIX + p;
    }
    ...
    function getRawUrl(path) {
        return `${CONFIG.GITHUB_RAW_BASE}/${CONFIG.GITHUB_REPO}/${CONFIG.GITHUB_BRANCH}/${libPath(path)}`;
    }
    ...
    function getApiUrl(path) {
        return `${CONFIG.GITHUB_API_BASE}/${CONFIG.GITHUB_REPO}/contents/${libPath(path)}`;
    }
    ...
    function getFileIcon(language) {
        const icons = { python: '🐍', json: '{}', javascript: 'JS' };
